...
public class BinaryTree<E> implements AbstractBinaryTree<E> {
    ...
    public String asIndentedPreOrder(int indent) {
        ...
        sb.append(key); // Добавляем ключ

        if (left != null || right != null) {
            sb.append(" "); // Добавляем пробел перед поддеревьями
        }
        ...
        if (right != null) {
            sb.append(" "); // Добавляем пробел перед правым поддеревом
    ...
    public void printTree() {
        ...
        for (int i = 0; i < maxLevel; i++) {
            int spaces = (int) Math.pow(2, maxLevel - i) - 1; // Вычисление отступов
